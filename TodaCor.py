...
def Energy_Toda_Vectorized(p,r):
    '''Computes the local energies of the Toda lattice 
    input: 
    p = velocities 
    r = stretcs '''
...
def Energy_Exp_Toda(p,r):
    ''' Compute the energy of the Toda lattice,
    input:
    p = velocities 
    r = stretcs '''
...
def evolution_periodic(p0,r0,time = [0,10],time_step = 0.1):
    ...
    the standar algorithm will be a leap-frog 
    '''
...
def one_step_evo(p,r,time_interval,time_step):
    ''' Integration step, compute the single evolution step
    p,r = initial data
...
def verlet(p,r,dt):
    ...
    p_mid = verlet_p(p,dt,r_exp_diff) 

   
    p_diff = np.ediff1d(p_mid, to_end = p_mid[0] - p_mid[-1])
    r_new = verlet_r(r,dt,p_diff)

    r_exp = np.exp(-r_new)
    r_exp_diff = np.ediff1d(r_exp, to_begin = r_exp[0] - r_exp[-1])
    p_new = verlet_p(p_mid,dt,r_exp_diff) 
...
def correlation_toda(p,r,p0,r0):
    ''' Compute the correlation function of the Toda lattice,
    input:
    p = velocies alogn the evolution
    r = streatches along the evolution
    p0,r0 = initial data '''
